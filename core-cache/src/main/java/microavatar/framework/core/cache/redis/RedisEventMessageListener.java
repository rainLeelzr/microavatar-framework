...
public class RedisEventMessageListener implements MessageListener {
    ...
    public void onMessage(Message message, byte[] pattern) {
        ...
        log.debug("接收到redis消息通知，topicName: {}，messageBody: {}，event：{}", topicName, messageBody, event);

        if (messageBody.length() == 0) {
            log.warn("messageBody.length()=0, 取消缓存更新任务！");
            return;
        }
        ...
        if (EventType.REMOVED.toString().equals(event)) {
            ...
            if (l1Cache == null) {
                log.warn("找不到cacheName为{}的l1Cache, 取消l1Cache清理任务", cacheName);
                return;
            }

            log.debug("正在清除l1Cache缓存{}中key={}的缓存", cacheName, key);
            l1Cache.evict(key);
        }
