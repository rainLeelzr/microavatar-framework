...
<body>    
    <form id="form1" method="post">
      <div style="padding-left:11px;padding-bottom:5px;">        <input name="id" id="id" class="mini-hidden" required="false"/>    
        ...
        <table style="table-layout:fixed;" class="table2">          <tr>
            ...
            <td class="tdcontent">              <input class="mini-textbox" id="resourceId" name="resourceId" emptyText="请输入资源id" vtype="maxLength:36" required="true" .if(false)onvalidation="onUniqueValidate".endif/>            </td>
          </tr>        </table>
    ...
    <script type="text/javascript">
      ...
      function SaveData() {
        ...
        $.ajax({
          url: "/auth/roleResource!save.action",
          type: 'post',
          data: { 
      ...
      function SetData(data) {
        ...
        if (data.action == "edit" || data.action == "view") {
          $.ajax({
            url: "/auth/roleResource!get.action",
            type:"post",
            data:{
              id:data.id
            },
      ...
      function openUpload(){
        mini.open({
          url : "/base/attachment!uploadPage.action?bizCode=roleResource",
          title : "上传附件",
          width : 1000,
          height : 600,
          onload : function() {
      ...
      function onUniqueValidate(e){
        if (e.isValid) {
          $.ajax({
            url:"/auth/roleResource!validateUnique.action",
            type:"post",
            data: {
