...
<body>    
    <form id="form1" method="post">
      <div style="padding-left:11px;padding-bottom:5px;">        <input name="id" id="id" class="mini-hidden" required="false"/>    
        ...
        <table style="table-layout:fixed;" class="table2">          <tr>
            <td class="tdtitle">角色id：</td>
            <td class="tdcontent">              <input class="mini-textbox" id="roleId" name="roleId" emptyText="请输入角色id" vtype="maxLength:36" required="true" .if(false)onvalidation="onUniqueValidate".endif/>            </td>
          </tr>          <tr>
            <td class="tdtitle">权限id：</td>
            <td class="tdcontent">              <input class="mini-textbox" id="sysPermissionId" name="sysPermissionId" emptyText="请输入权限id" vtype="maxLength:36" required="true" .if(false)onvalidation="onUniqueValidate".endif/>            </td>
          </tr>        </table>
    ...
    <script type="text/javascript">
      ...
      function SaveData() {
        ...
        $.ajax({
          url: "/auth/roleSysPermission!save.action",
          type: 'post',
          data: { 
      ...
      function SetData(data) {
        ...
        if (data.action == "edit" || data.action == "view") {
          $.ajax({
            url: "/auth/roleSysPermission!get.action",
            type:"post",
            data:{
              id:data.id
            },
      ...
      function openUpload(){
        mini.open({
          url : "/base/attachment!uploadPage.action?bizCode=roleSysPermission",
          title : "上传附件",
          width : 1000,
          height : 600,
          onload : function() {
      ...
      function onUniqueValidate(e){
        if (e.isValid) {
          $.ajax({
            url:"/auth/roleSysPermission!validateUnique.action",
            type:"post",
            data: {
