...
<body>    
    <form id="form1" method="post">
      <div style="padding-left:11px;padding-bottom:5px;">        <input name="id" id="id" class="mini-hidden" required="false"/>    
        ...
        <table style="table-layout:fixed;" class="table2">          <tr>
            ...
            <td class="tdtitle">组织id：</td>
            <td class="tdcontent">              <input class="mini-textbox" id="orgId" name="orgId" emptyText="请输入组织id" vtype="maxLength:36" required="true" .if(false)onvalidation="onUniqueValidate".endif/>            </td>
          </tr>        </table>
    ...
    <script type="text/javascript">
      ...
      function openUpload(){
        mini.open({
          url : "/base/attachment!uploadPage.action?bizCode=userOrg",
          title : "上传附件",
          width : 1000,
          height : 600,
          onload : function() {
      ...
      function onUniqueValidate(e){
        if (e.isValid) {
          $.ajax({
            url:"/auth/userOrg!validateUnique.action",
            type:"post",
            data: {
